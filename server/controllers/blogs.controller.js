...
const createBlog = async (req, res, next) => {
    try {
        const { title, text, imagePath, ...rest } = req.body;
        const content = text;
...
const readBlogs = async (req, res, next) => {
    try {
        ...
        const stype = req.query.sortType ? Number(req.query.sortType) : 1;
        let sfield = "blog.createDate";
        let dir = "DESC";

        if (stype === 2) {
            sfield = "blog.createDate";
            dir = "ASC";
        } else if (stype === 3) {
            sfield = "like";
            dir = "DESC";
...
const updateBlog = async (req, res, next) => {
    try {
        const { id } = req.params;
        const { title, content, imagePath, ...rest } = req.body;
